...
WEBGL_GAL::WEBGL_GAL( const KIGFX::VC_SETTINGS& aVcSettings, GAL_DISPLAY_OPTIONS& aDisplayOptions,
                        wxWindow* aParent,
        ...
        m_lockClientCookie( 0 )
{
    ...
    m_compositor = new WEBGL_COMPOSITOR;
    ...
    evtHandler->Bind( wxEVT_SET_CURSOR, &WEBGL_GAL::onSetNativeCursor, this );
    evtHandler->Bind( wxEVT_PAINT, &WEBGL_GAL::onPaint, this );

    // Mouse events are skipped to the parent
    evtHandler->Bind( wxEVT_MOTION, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_LEFT_DOWN, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_LEFT_UP, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_LEFT_DCLICK, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_MIDDLE_DOWN, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_MIDDLE_UP, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_MIDDLE_DCLICK, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_RIGHT_DOWN, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_RIGHT_UP, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_RIGHT_DCLICK, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX1_DOWN, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX1_UP, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX1_DCLICK, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX2_DOWN, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX2_UP, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_AUX2_DCLICK, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_MOUSEWHEEL, &WEBGL_GAL::skipMouseEvent, this );
    evtHandler->Bind( wxEVT_MAGNIFY, &WEBGL_GAL::skipMouseEvent, this );

#if defined _WIN32 || defined _WIN64
    evtHandler->Bind( wxEVT_ENTER_WINDOW, &WEBGL_GAL::skipMouseEvent, this );
#endif

    evtHandler->Bind( wxEVT_GESTURE_ZOOM, &WEBGL_GAL::skipGestureEvent, this );
    evtHandler->Bind( wxEVT_GESTURE_PAN, &WEBGL_GAL::skipGestureEvent, this );
...
wxString WEBGL_GAL::CheckFeatures( GAL_DISPLAY_OPTIONS& aOptions )
{
    ...
    KIGFX::WEBGL_GAL* opengl_gal = nullptr;

    try
    {
        KIGFX::VC_SETTINGS dummy;
        opengl_gal = new KIGFX::WEBGL_GAL( dummy, aOptions, testFrame );
...
void WEBGL_GAL::PostPaint( wxPaintEvent& aEvent )
{
...
bool WEBGL_GAL::updatedGalDisplayOptions( const GAL_DISPLAY_OPTIONS& aOptions )
{
...
double WEBGL_GAL::getWorldPixelSize() const
{
...
VECTOR2D WEBGL_GAL::getScreenPixelSize() const
{
...
void WEBGL_GAL::SetMinLineWidth( float aLineWidth )
{
...
void WEBGL_GAL::LockContext( int aClientCookie )
{
...
void WEBGL_GAL::UnlockContext( int aClientCookie )
{
...
void WEBGL_GAL::DrawLine( const VECTOR2D& aStartPoint, const VECTOR2D& aEndPoint )
{
...
void WEBGL_GAL::DrawSegment( const VECTOR2D& aStartPoint, const VECTOR2D& aEndPoint,
                              double aWidth )
{
...
void WEBGL_GAL::drawSegment( const VECTOR2D& aStartPoint, const VECTOR2D& aEndPoint, double aWidth,
                              bool aReserve )
{
...
void WEBGL_GAL::DrawCircle( const VECTOR2D& aCenterPoint, double aRadius )
{
    drawCircle( aCenterPoint, aRadius );
}


void WEBGL_GAL::DrawHoleWall( const VECTOR2D& aCenterPoint, double aHoleRadius,
                               double aWallWidth )
{
    if( m_isFillEnabled )
    {
...
void WEBGL_GAL::drawCircle( const VECTOR2D& aCenterPoint, double aRadius, bool aReserve )
{
    if( m_isFillEnabled )
    {
        if( aReserve )
...
void WEBGL_GAL::DrawArc( const VECTOR2D& aCenterPoint, double aRadius,
...
void WEBGL_GAL::DrawArcSegment( const VECTOR2D& aCenterPoint, double aRadius,
                                 const EDA_ANGLE& aStartAngle, const EDA_ANGLE& aAngle,
...
void WEBGL_GAL::DrawRectangle( const VECTOR2D& aStartPoint, const VECTOR2D& aEndPoint )
{
...
void WEBGL_GAL::DrawSegmentChain( const std::vector<VECTOR2D>& aPointList, double aWidth )
{
    drawSegmentChain(
            [&]( int idx )
            {
...
void WEBGL_GAL::DrawSegmentChain( const SHAPE_LINE_CHAIN& aLineChain, double aWidth )
{
...
void WEBGL_GAL::DrawPolyline( const std::deque<VECTOR2D>& aPointList )
{
    drawPolyline(
            [&]( int idx )
            {
...
void WEBGL_GAL::DrawPolyline( const std::vector<VECTOR2D>& aPointList )
{
    drawPolyline(
            [&]( int idx )
            {
...
void WEBGL_GAL::DrawPolyline( const VECTOR2D aPointList[], int aListSize )
{
    drawPolyline(
            [&]( int idx )
            {
...
void WEBGL_GAL::DrawPolyline( const SHAPE_LINE_CHAIN& aLineChain )
{
    auto numPoints = aLineChain.PointCount();
...
void WEBGL_GAL::DrawPolylines( const std::vector<std::vector<VECTOR2D>>& aPointList )
{
    int lineQuadCount = 0;
...
void WEBGL_GAL::DrawPolygon( const std::deque<VECTOR2D>& aPointList )
{
...
void WEBGL_GAL::DrawPolygon( const VECTOR2D aPointList[], int aListSize )
{
...
void WEBGL_GAL::drawTriangulatedPolyset( const SHAPE_POLY_SET& aPolySet,
...
void WEBGL_GAL::DrawPolygon( const SHAPE_POLY_SET& aPolySet, bool aStrokeTriangulation )
{
...
void WEBGL_GAL::DrawPolygon( const SHAPE_LINE_CHAIN& aPolygon )
{
...
void WEBGL_GAL::DrawCurve( const VECTOR2D& aStartPoint, const VECTOR2D& aControlPointA,
...
void WEBGL_GAL::BitmapText( const wxString& aText, const VECTOR2I& aPosition,
...
void WEBGL_GAL::ResizeScreen( int aWidth, int aHeight )
{
...
bool WEBGL_GAL::Show( bool aShow )
{
...
void WEBGL_GAL::Rotate( double aAngle )
{
    m_currentManager->Rotate( aAngle, 0.0f, 0.0f, 1.0f );
}


void WEBGL_GAL::Translate( const VECTOR2D& aVector )
{
    m_currentManager->Translate( aVector.x, aVector.y, 0.0f );
}


void WEBGL_GAL::Scale( const VECTOR2D& aScale )
{
...
void WEBGL_GAL::Restore()
{
    m_currentManager->PopMatrix();
}


int WEBGL_GAL::BeginGroup()
{
    m_isGrouping = true;
...
void WEBGL_GAL::DrawGroup( int aGroupNumber )
{
...
void WEBGL_GAL::ChangeGroupColor( int aGroupNumber, const COLOR4D& aNewColor )
{
    auto group = m_groups.find( aGroupNumber );
...
void WEBGL_GAL::ChangeGroupDepth( int aGroupNumber, int aDepth )
{
    auto group = m_groups.find( aGroupNumber );
...
void WEBGL_GAL::DeleteGroup( int aGroupNumber )
{
...
void WEBGL_GAL::SetTarget( RENDER_TARGET aTarget )
{
    switch( aTarget )
    {
    default:
...
RENDER_TARGET WEBGL_GAL::GetTarget() const
{
    return m_currentTarget;
}


void WEBGL_GAL::ClearTarget( RENDER_TARGET aTarget )
{
...
bool WEBGL_GAL::HasTarget( RENDER_TARGET aTarget )
{
    switch( aTarget )
    {
    default:
...
bool WEBGL_GAL::SetNativeCursorStyle( KICURSOR aCursor, bool aHiDPI )
{
...
void WEBGL_GAL::onSetNativeCursor( wxSetCursorEvent& aEvent )
{
#if wxCHECK_VERSION( 3, 3, 0 )
...
void WEBGL_GAL::DrawCursor( const VECTOR2D& aCursorPosition )
{
...
void WEBGL_GAL::drawLineQuad( const VECTOR2D& aStartPoint, const VECTOR2D& aEndPoint,
                               const bool aReserve )
{
...
void WEBGL_GAL::reserveLineQuads( const int aLineCount )
{
    m_currentManager->Reserve( 6 * aLineCount );
}


void WEBGL_GAL::drawSemiCircle( const VECTOR2D& aCenterPoint, double aRadius, double aAngle )
{
    if( m_isFillEnabled )
    {
        m_currentManager->Color( m_fillColor.r, m_fillColor.g, m_fillColor.b, m_fillColor.a );
...
void WEBGL_GAL::drawFilledSemiCircle( const VECTOR2D& aCenterPoint, double aRadius, double aAngle )
{
    Save();
...
void WEBGL_GAL::drawStrokedSemiCircle( const VECTOR2D& aCenterPoint, double aRadius, double aAngle,
                                        bool aReserve )
{
...
void WEBGL_GAL::drawPolygon( GLdouble* aPoints, int aPointCount )
{
    if( m_isFillEnabled )
    {
        m_currentManager->Shader( SHADER_NONE );
...
void WEBGL_GAL::drawPolyline( const std::function<VECTOR2D( int )>& aPointGetter, int aPointCount,
                               bool aReserve )
{
...
void WEBGL_GAL::drawSegmentChain( const std::function<VECTOR2D( int )>& aPointGetter,
...
int WEBGL_GAL::drawBitmapChar( unsigned long aChar, bool aReserve )
{
...
void WEBGL_GAL::drawBitmapOverbar( double aLength, double aHeight, bool aReserve )
{
...
std::pair<VECTOR2D, float> WEBGL_GAL::computeBitmapTextSize( const UTF8& aText ) const
{
...
void WEBGL_GAL::onPaint( wxPaintEvent& aEvent )
{
    PostPaint( aEvent );
}


void WEBGL_GAL::skipMouseEvent( wxMouseEvent& aEvent )
{
...
void WEBGL_GAL::skipGestureEvent( wxGestureEvent& aEvent )
{
...
unsigned int WEBGL_GAL::getNewGroupNumber()
{
...
void WEBGL_GAL::setupShaderParameters()
{
...
void CALLBACK VertexCallback( GLvoid* aVertexPtr, void* aData )
{
    ...
    WEBGL_GAL::TessParams* param = static_cast<WEBGL_GAL::TessParams*>( aData );
...
void WEBGL_GAL::EnableDepthTest( bool aEnabled )
{
...
void WEBGL_GAL::ComputeWorldScreenMatrix()
{
    computeWorldScale();
...
void WEBGL_GAL::DrawGlyph( const KIFONT::GLYPH& aGlyph, int aNth, int aTotal )
{
    if( aGlyph.IsStroke() )
    {
...
void WEBGL_GAL::DrawGlyphs( const std::vector<std::unique_ptr<KIFONT::GLYPH>>& aGlyphs )
{
    if( aGlyphs.empty() )
        return;
